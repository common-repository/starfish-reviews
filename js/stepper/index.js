...
const DEFAULT_OPTIONS = {
	...
	animation: true,
	selectors: {
		steps: '.srm-funnel-step',
		trigger: '.srm-funnel-step-trigger',
		stepper: '#srm-funnel'
	}
}
...
class Stepper {
	constructor (element, _options = {}) {
		...
		this.options = jQuery.extend(DEFAULT_OPTIONS, _options);

		this.options.selectors = jQuery.extend(DEFAULT_OPTIONS.selectors, this.options.selectors);

		if (this.options.linear) {
			this._element.classList.add(ClassName.LINEAR)
		}

		this._steps = [].slice.call(document.querySelectorAll(this.options.selectors.steps))
