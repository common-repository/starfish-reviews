...
function srm_start_plugin_migrations() {
	if ( $_POST['retry'] ) {
		...
		delete_transient( 'srm_successful_migrations' );
...
function srm_export_feedback() {
	...
	echo wp_json_encode(
		array(
			'type'      => 'success',
			'url'       => SRM_PLUGIN_URL . '/exports/' . $filename,
			'file_name' => basename(
				$filename,
				'.csv'
			),
 ...
 * Purge Log file
 */
function srm_purge_logfile() {
	// Verify if the nonce is valid
	if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'srm_settings' ) ) {
		echo wp_json_encode( array( 'type' => 'failed' ) );
		wp_die();
	}
	$purged = SRM_LOGGING::purgeLogs();
	if ( $purged ) {
		echo wp_json_encode(
			array(
				'type'    => 'success',
...
add_action( 'wp_ajax_srm-settings-logs-purge', 'srm_purge_logfile' );

/**
 * Refresh Log file
 */
function srm_refresh_logfile() {
	if ( file_exists( SRM_LOG_FILE ) ) {
		$logs = file_get_contents( SRM_LOG_FILE );
		if ( ! empty( $logs ) ) {
...
add_action( 'wp_ajax_srm-settings-logs-refresh', 'srm_refresh_logfile' );

/**
 * Refresh Plan Limits
 */
function srm_refresh_plan_limits() {
	if ( delete_transient( SRM_TRAN_PLAN_LIMITS ) ) {
		$srm_error_cya = '01';
		$plan_limits   = SRM_FREEMIUS::srm_get_plan_limit();
		SRM_LOGGING::addEntry(
			array(
				'level'   => SRM_LOGGING::SRM_INFO,
				'action'  => 'Successfully refreshed Plan Limits',
				'message' => null,
				'code'    => 'SRM_B_RPL_A_' . $srm_error_cya,
			)
		);
		$results = wp_json_encode(
			array(
				'type'    => 'success',
				'content' => [
					'collections' => $plan_limits['collections'],
					'profiles'    => $plan_limits['profiles'],
					'funnels'     => $plan_limits['funnels'],
				]
			)
		);
	} else {
...
add_action( 'wp_ajax_starfish-execute-plan-limit-refresh', 'srm_refresh_plan_limits' );

/**
 * Get Transient for front-end usage.
 *
 * @return void
 */
function srm_get_transient() {
	$transient = get_transient( $_POST['transient'] );
	if ( $transient ) {
		$results = wp_json_encode(
			array(
				'type'    => 'success',
...
add_action( 'wp_ajax_starfish-get-transient', 'srm_get_transient' );
